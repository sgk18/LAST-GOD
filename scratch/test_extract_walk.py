...
# Frame X bounding boxes (approximate split)
frame_bounds = [
    (175, 395),
    (405, 649),
    (655, 895),
...
os.makedirs(r"c:\projects\LAST-GOD\scratch\walk_test", exist_ok=True)

CANVAS_WIDTH = 320
CANVAS_HEIGHT = 704
FEET_BASELINE_Y = 688

SCALE = 643.0 / 393.0  # match Idle character height
...
for i, (x1, x2) in enumerate(frame_bounds):
    ...
    # Place on 320x704 canvas
    canvas = np.zeros((CANVAS_HEIGHT, CANVAS_WIDTH, 4), dtype=np.uint8)
    
    # Feet contact at FEET_BASELINE_Y
    # Top placement:
    paste_y = FEET_BASELINE_Y - new_h
    # Horizontal center:
    paste_x = (CANVAS_WIDTH - new_w) // 2
    
    # Paste into canvas with bounds check
    ...
    out_path = rf"c:\projects\LAST-GOD\scratch\walk_test\Aeron_Walk_{i+1:02d}.png"
    out_img.save(out_path)
    print(f"Saved {out_path}: placed at x={paste_x}, y={paste_y}, size=({new_w}, {new_h})")
